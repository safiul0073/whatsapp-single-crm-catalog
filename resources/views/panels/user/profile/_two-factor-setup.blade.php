        ...
        @if (setting('require_2fa_for_users', false) && !$user->hasConfirmedTwoFactor())
            <div class="rounded-xl border border-warning/30 bg-warning/10 p-4 mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <span
                        class="text-sm font-medium text-warning">{{ __('Two-factor authentication is required by your administrator.') }}</span>
                </div>
            </div>
        @endif
        ...
        @if ($user->hasConfirmedTwoFactor())
            ...
            <div class="rounded-xl border border-success/30 bg-success/10 p-4 mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <span
                        class="text-sm font-medium text-success">{{ __('Two-factor authentication is enabled.') }}</span>
                </div>
            </div>
            ...
            @if (session('recovery_codes'))
                <div class="rounded-xl border border-warning/30 bg-warning/10 p-4 mb-4">
                    <div class="flex items-center gap-2 mb-3">
                        <i class="ph ph-warning text-warning text-lg"></i>
                        <span
                            class="text-sm font-medium text-warning">{{ __('Save these recovery codes in a secure location.') }}</span>
                    </div>
            ...
            @else
                <p class="text-sm text-neutral-500">
                    {{ __('Two-factor authentication is required and cannot be disabled.') }}</p>
            @endunless
        @else
            {{-- 2FA is not enabled --}}
